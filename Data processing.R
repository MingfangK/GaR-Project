...
time_custom_plot <- function(data){
  
  # En el df siempre debe estar la fecha en la primera columna y la serie en la segunda.
  # Para evitar problemas que los df contengan unicamente esas dos columnas
...
GDP_definitive_series <- GDP_all_series  %>%
             ...
             time_custom_plot(FCI_col) +
               ggtitle("Índice de condiciones financieras Colombia 1991 - 2016")
           
           
             time_custom_plot(GDP_all_series[-(1:4),c(1,3)]) +
                   ggtitle("Crecimiento del PIB 1994 - 2007- Base 1994")
...
#Pendiente COLCAP empalmado desde el 2001, inflación y variables de finanzas públicas
